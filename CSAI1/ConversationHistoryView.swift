    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                VStack(spacing: 0) {
                    // Show/hide search bar with a fade
                    if showSearch {
                        searchBar()
                            .transition(.opacity) // simpler fade
                            .animation(.easeInOut, value: showSearch)
                    }
                    
                    // The list of pinned + unpinned conversations
                    conversationList()
                }
            ...
            // Single magnifying glass button toggles search bar
            .toolbar {
...
extension ConversationHistoryView {
    ...
    /// A simple search bar at the top with a subtle glow
    ...
    /// The main list of pinned + unpinned conversations
    ...
    private func conversationRow(_ convo: Conversation) -> some View {
        ...
        .contextMenu {
            Button("Select") { onSelectConversation(convo) }
