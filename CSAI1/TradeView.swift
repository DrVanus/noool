...
struct TradeView: View {
    ...
    // MARK: - Init
    /// Pass `showBackButton: true` if you want the back button visible (e.g. user came from a detail screen).
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // 3) Chart
            chartSection
                .zIndex(1)            // Ensure chart is below the interval picker
                .frame(height: 240)
                .clipped()            // Prevents any chart overlay from exceeding its frame
            
            // 4) Interval Picker
            intervalPicker
                .zIndex(2)           // The picker is on top, so taps always register
    ...
    private var navBar: some View {
        ZStack {
            // Left: Back (only if showBackButton == true)
            if showBackButton {
                HStack {
                    Button {
    ...
    private var priceRow: some View {
        HStack {
            ...
            case 0:
                // 0 means "still loading"
                Text("Loading...")
                    .foregroundColor(.yellow)
                    .font(.system(size: 20, weight: .bold))
            case -1:
                // -1 means "error fetching"
    ...
    private var chartTypeToggle: some View {
        HStack(spacing: 16) {
            ...
            // CryptoSage AI button (circle placeholder icon)
            Button {
            ...
            // TradingView button (circle placeholder icon)
            Button {
        ...
        VStack(spacing: 12) {
            ...
            HStack(spacing: 0) {
                ...
                Button {
                    ...
                    Text("Sell")
                        .font(.system(size: 14, weight: .semibold))
                        .foregroundColor(selectedSide == .sell ? .black : .white)
                        .padding(.vertical, 10)
                        .frame(maxWidth: .infinity)
...
/// Coin picker with black background, a search bar, a custom row, and checkmarks.
...
class OrderBookViewModel: ObservableObject {
    ...
    private func fetchOrderBook(pair: String) {
        // e.g. GET https://api.exchange.coinbase.com/products/BTC-USD/book?level=2
...
// MARK: - PriceViewModel
///
/// Uses Coinbase for the live price: GET https://api.exchange.coinbase.com/products/<symbol>-USD/ticker
/// If it fails, sets `currentPrice = -1`.
///
    ...
    var body: some View {
        VStack {
            if vm.isLoading {
                ...
                // Insert your real candlestick or line chart code here
        ...
        // <--- IMPORTANT: Re-fetch when 'symbol' changes
        .onChange(of: symbol) { newSymbol in
            vm.fetchBinanceData(symbol: newSymbol,
                                interval: interval.binanceInterval,
                                limit: interval.binanceLimit)
        }
        // Also re-fetch when interval changes
