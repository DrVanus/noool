...
// ──────────────────────────────────────────────────────────────────────────
// NEW: PaymentMethod struct to represent user’s wallet/exchange
...
struct TradeView: View {
    ...
    // NEW: Slider from 0..1 so user can pick a fraction of “balance”
    ...
    var body: some View {
        ...
        // NEW: Payment method picker as a sheet
        .sheet(isPresented: $isPaymentMethodPickerPresented) {
            EnhancedPaymentMethodPickerView(currentMethod: $selectedPaymentMethod) { newMethod in
    ...
    private var navBar: some View {
        ZStack {
            ...
            HStack {
                ...
                // ─────────────────────────────────────────────────────────────────────
                // ADJUSTMENT: Payment method button moved here to the left side
                Button {
    ...
    // NEW: A comma-based price formatter
    ...
    private var tradeForm: some View {
        VStack(spacing: 12) {
            ...
            // Quantity row + quick % on the right
            HStack(spacing: 8) {
                Text("Quantity:")
                    .foregroundColor(.white)
                ...
                // ─────────────────────────────────────────────────────────────────────
                // ADJUSTMENT: Smaller font (size 10) & narrower horizontal padding
                HStack(spacing: 0) {
            ...
            Button {
                ...
                Text("\(selectedSide.rawValue.capitalized) \(symbol.uppercased())")
                    .font(.headline)
                    // SELL => white text on red
                    // BUY => black text on yellow
...
// NEW: Payment Method Picker, similar to EnhancedCoinPickerView
struct EnhancedPaymentMethodPickerView: View {
    ...
    var filteredMethods: [PaymentMethod] {
        if searchText.isEmpty { return allMethods }
