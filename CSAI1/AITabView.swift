...
// MARK: - ChatBubble
/// A chat bubble (or no-bubble) view for a single ChatMessage:
/// - AI: White text, left-aligned, no visible timestamp (shown on long press).
/// - User: Yellow bubble, black text, right-aligned, no visible timestamp (shown on long press).
/// - Error: Red bubble, white text, right-aligned, no visible timestamp (shown on long press).
struct ChatBubble: View {
    ...
    /// AI messages: white text, left-aligned, no visible timestamp.
    /// Timestamp appears in a context menu on long press.
    ...
    /// User or error messages: bubble with text, right-aligned, no visible timestamp.
    /// Timestamp appears in a context menu on long press.
...
struct AITabView: View {
    ...
    /// A larger pool of prompts to pick from
    ...
    /// The set of prompts currently shown in the quick-reply bar
    ...
    var body: some View {
        NavigationView {
            ...
            chatBodyView
                ...
                .onAppear {
                    loadConversations()
                    // If no active conversation is set, default to the first
                    if activeConversationID == nil, let first = conversations.first {
                        activeConversationID = first.id
                    }
                    // Generate a random set of quick replies
...
extension AITabView {
    ...
    /// Horizontal quick replies bar, plus a "Load More" button on the right
    private func quickReplyBar() -> some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 10) {
                // Show each prompt button
                ...
                // A button to shuffle new random prompts
                Button(action: generateRandomPrompts) {
    ...
    /// Randomly picks a few prompts from `allPrompts`
    private func generateRandomPrompts() {
        let randomCount = min(allPrompts.count, 4) // e.g. show up to 4
        quickReplies = Array(allPrompts.shuffled().prefix(randomCount))
    }
