...
struct CoinDetailView: View {
    ...
    private var navBar: some View {
        ...
        .overlay(
            Rectangle()
                .fill(Color.white.opacity(0.05))  // Less intrusive line
                .frame(height: 1),
            alignment: .bottom
        )
    }
...
struct CoinDetailCustomChart: View {
    ...
    private var chartContent: some View {
        ...
        // Slight top margin so the line doesn't hug the top
        ...
        // Decide how many x-axis labels we want
                ...
                .annotation(position: .top) {
                    VStack(spacing: 2) {
                        if interval.hideCrosshairTime {
                            ...
                            // For short intervals, show local time with minutes
                            switch interval {
                            case .oneMin, .fiveMin:
                                Text(formatTimeLocalHM(cVal.date)) // e.g. "6:25 PM"
                                    .font(.caption2)
                                    .foregroundColor(.white)
                            case .fifteenMin, .thirtyMin, .oneHour, .fourHour:
                                Text(formatTimeLocal(cVal.date))   // e.g. "6 PM"
                                    .font(.caption2)
                                    .foregroundColor(.white)
    ...
    private func formatAxisDate(_ date: Date) -> String {
        ...
        case .fifteenMin, .thirtyMin, .oneHour, .fourHour:
            df.dateFormat = "ha" // e.g. "2 PM"
        case .oneDay, .oneWeek, .oneMonth, .threeMonth:
            // If day == 1, just "MMM", else "MMM d"
            let day = Calendar.current.component(.day, from: date)
            df.dateFormat = (day == 1) ? "MMM" : "MMM d"
        default:
            // 1Y, 3Y, ALL => "MMM yyyy" e.g. "Jan 2023"
            df.dateFormat = "MMM yyyy"
        }
        return df.string(from: date)
    }
    
    /// For crosshair on 1m/5m intervals: "h:mm a" e.g. "6:25 PM"
    ...
    /// For crosshair on 15m–4H intervals: "ha" e.g. "6 PM"
